...
def make_data(
    recipe: dict[str, Any],
    device: torch.device,
    normalizer: tuple[Tensor, Tensor] | None = None,
...
class OPFModel(nn.Module):
    ...
    def __init__(
        self, observation_dim: int, state_dim: int, num_factors: int, hidden: int
    ) -> None:
        super().__init__()
...
def _geometry(
    ...
    basis = (
        projection.analysis_basis().detach().double().reshape(features.shape[-1], -1)
    )
    gram = basis @ basis.T
    ...
    return {
        ...
        "state_roundtrip_rmse": float(
            (reconstructed - features).square().mean().sqrt()
        ),
    }
...
def evaluate(
    ...
    ridge: float,
    split_name: str = "test",
    eval_data: dict[str, Tensor] | None = None,
) -> dict[str, float]:
    model.eval()
    obs, states = data["observations"], data["states"]
    train_first, train_last = recipe["data"]["split"]["train_ids"]
    if split_name not in ("validation", "test"):
        raise ValueError("evaluation split must be validation or test")
    test_first, test_last = recipe["data"]["split"][f"{split_name}_ids"]
    evaluated = data if eval_data is None else eval_data
    with torch.inference_mode():
        train_features = model.online_encoder(
            obs[train_first : train_last + 1].reshape(-1, obs.shape[-1])
        ).double()
        train_states = (
            states[train_first : train_last + 1].reshape(-1, states.shape[-1]).double()
        )
        ...
        def decode(features: Tensor) -> Tensor:
            return (
                (features.double() - feature_mean) / feature_std
            ) @ coefficients + state_mean

        test_obs = evaluated["observations"][test_first : test_last + 1]
        test_states = evaluated["states"][test_first : test_last + 1]
        test_controls = evaluated["controls"][test_first : test_last + 1]
        ...
        for horizon in range(1, max(HORIZONS) + 1):
            action = test_controls[:, horizon - 1 : horizon - 1 + start_count].reshape(
                -1, 1
            )
...
def train_one(
...
) -> tuple[dict[str, float | str | int], list[dict[str, float]], OPFModel]:
    ...
    model = OPFModel(
        int(recipe["system"]["observation_dim"]), state_dim, num_factors, args.hidden
    ).to(args.device)
    init_output_scale = float(getattr(args, "init_output_scale", 1.0))
    if init_output_scale != 1.0:
        with torch.no_grad():
            last_layer = model.online_encoder[-1]
            assert isinstance(last_layer, nn.Linear)
            last_layer.weight.mul_(init_output_scale)
            last_layer.bias.mul_(init_output_scale)
            model.target_encoder.load_state_dict(model.online_encoder.state_dict())
    ...
    if (
        abs(parameter_count(model) - planned_parameters) / planned_parameters
        > tolerance
    ):
    ...
    orthogonality_weight = getattr(args, "orthogonality_weight", None)
    if orthogonality_weight is None:
        orthogonality_weight = losses["projector_orthogonality"]["weight"]
    factor_min_std = getattr(args, "factor_min_std", None)
    if factor_min_std is None:
        factor_min_std = losses["factor_activity"]["min_std"]
    encoder_min_std = getattr(args, "encoder_min_std", None)
    if encoder_min_std is None:
        encoder_min_std = losses["online_encoder_activity"]["min_std"]
    ...
    factor_active_updates = 0
    encoder_active_updates = 0
    started = time.perf_counter()
    model.train()
    for step in range(1, args.steps + 1):
        index = torch.randint(
            len(context), (args.batch_size,), generator=sampling, device=args.device
        )
        ...
        objective = jepa_anything_objective(
            ...
            * float(losses["factor_activity"]["weight"]),
            encoder_variance_weight=floor_weight
            * float(losses["online_encoder_activity"]["weight"]),
            sigreg_weight=sigreg_weight,
            sigreg=regularizer,
            sigreg_embeddings=context_state,
            factor_min_std=float(factor_min_std),
            encoder_min_std=float(encoder_min_std),
        )
        factor_active_updates += bool(objective.factor_activity.detach() > 0)
        encoder_active_updates += bool(objective.encoder_variance.detach() > 0)
        ...
        if step == 1 or step % args.log_every == 0 or step == args.steps:
            ...
            print(
                f"{variant} seed={seed} step={step} loss={record['loss']:.6f}",
                flush=True,
            )
    if args.device.type == "cuda":
    ...
    metrics: dict[str, float | str | int] = {
        ...
        "factor_floor_active_fraction": factor_active_updates / args.steps,
        "encoder_floor_active_fraction": encoder_active_updates / args.steps,
        **evaluate(
            model,
            data,
            recipe,
            args.probe_ridge,
            getattr(args, "evaluation_split", "test"),
        ),
    }
...
def main() -> None:
    ...
    parser.add_argument("--orthogonality-weight", type=float)
    parser.add_argument("--factor-min-std", type=float)
    parser.add_argument("--encoder-min-std", type=float)
    parser.add_argument("--init-output-scale", type=float, default=1.0)
    parser.add_argument(
    ...
    if (
        ...
        raise ValueError(
            "steps, batch size, hidden width, and log interval must be positive"
        )
    if (
        not 0 <= args.ema_momentum < 1
        or args.sigreg_weight < 0
        or args.probe_ridge <= 0
    ):
    ...
    if (
        any(
            ...
            for value in (
                ...
                args.factor_min_std,
                args.encoder_min_std,
            )
        )
        or args.init_output_scale <= 0
    ):
        raise ValueError(
            "loss weights and thresholds must be non-negative; init scale positive"
        )
    ...
    for variant in variants:
        for seed in seeds:
            metrics, history, _model = train_one(variant, seed, data, recipe, args)
